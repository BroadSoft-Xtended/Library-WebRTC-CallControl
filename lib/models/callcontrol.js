...
function CallControl(eventbus, debug, configuration, sipstack, sound) {
  ...
  var updateClasses = function(){
    self.classes = [
      self.visible ? 'callcontrol-shown' : 'callcontrol-hidden',
      sipstack.callState,
      configuration.enableCallControl ? 'enableCallControl' : ''
    ].concat(configuration.views);
  }

  self.props = {'destination': true, 'classes': true, 'visible': {
    onSet: function(){
      updateClasses();
    }
  }};
  ...
  self.listeners = function(configurationDatabinder, sipstackDatabinder) {
    ...
    configurationDatabinder.onModelPropChange(['enableCallControl', 'views'], function(){
      updateClasses();
    });
